function scr_solid_obj(){
	if (status&STA_ONOBJ)
	{
		...
		{
			with(player) scr_solid_move_player();
			return 1;
		}
	}
...
function scr_solid_move_player(){
	if (routine>2 || platform_ID==-1) exit; // Don't collide if in death
	
	y = platform_ID.bbox_top-height;
	x -= (platform_ID.prev_x - platform_ID.x)
}
